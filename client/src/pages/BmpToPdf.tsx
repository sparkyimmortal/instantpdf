...
export default function BmpToPdf() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const response = await pdfFetch("/api/pdf/bmp-to-pdf", {
        method: "POST",
        body: formData,
      });
      ...
      toast({
        title: "Success!",
        description: "BMP images converted to PDF successfully.",
      });
    } catch (err) {
      console.error(err);
      setIsProcessing(false);
  ...
  if (isComplete) {
    return (
      <ToolLayout
        title="BMP to PDF"
        description="Convert BMP images to a PDF document"
      >
        <div className="flex flex-col items-center justify-center p-12 text-center space-y-6">
          ...
          <p className="text-muted-foreground max-w-md">
            Your BMP images have been converted to a PDF document.
          </p>
    ...
    >
      {files.length === 0 ? (
        ...
        >
          ...
          <h3 className="text-xl font-semibold mb-2">Select BMP Images</h3>
          <p className="text-muted-foreground mb-6">or drop BMP files here</p>
