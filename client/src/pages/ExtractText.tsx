...
import { Upload, FileText, Download, Loader2, ArrowLeft, Check, Copy } from "lucide-react";
...
export default function ExtractText() {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const response = await pdfFetch("/api/pdf/extract-text", {
        method: "POST",
        body: formData,
      });
      ...
      // Fetch the text content for preview
      try {
        const textResponse = await pdfFetch(data.downloadUrl);
        if (textResponse.ok) {
          const text = await textResponse.text();
          setExtractedText(text.slice(0, 2000) + (text.length > 2000 ? "..." : ""));
        }
      } catch {
        // Preview failed, but download still works
      }
      ...
      <main className="container px-4 py-8 md:py-12">
        ...
        >
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center h-16 w-16 rounded-2xl bg-teal-500/10 text-teal-500 mb-4">
            ...
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              Convert your PDF document to plain text. Perfect for copying content or creating searchable documents.
            </p>
          </div>
          ...
          {!file ? (
            <Card
              ...
              data-testid="dropzone-extract-text"
            >
              <input {...getInputProps()} data-testid="input-file-extract-text" />
              <div className="flex flex-col items-center justify-center text-center">
                ...
                <Button variant="outline" data-testid="button-browse-extract-text">
                  Select PDF
                </Button>
              </div>
            </Card>
